  ...
  return (
    <section className="max-w-7xl mx-auto px-6 py-20">
      <div className="flex items-end justify-between mb-8">
        ...
        >
          {t("categoryShowcase.allCategories")}
        </Link>
      </div>
      ...
      <div className="grid md:grid-cols-2">
        ...
        >
          <img
            src={large.image}
            alt={t(`categoryShowcase.categories.${large.key}`)}
          ...
          <div className="absolute bottom-8 left-8 right-8">
            <p className="text-white/70 text-xs font-semibold tracking-widest uppercase mb-2">
              {large.count} {t("categoryShowcase.stylesSuffix")}
            </p>
            <p className="font-serif text-3xl font-bold text-white mb-4">
              {t(`categoryShowcase.categories.${large.key}`)}
            </p>
        ...
        <div className="grid grid-cols-2">
          {small.map((category) => (
            ...
            >
              <img
                src={category.image}
                alt={t(`categoryShowcase.categories.${category.key}`)}
                className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-105"
              />
              ...
              <div className="absolute bottom-4 left-4">
                <p className="text-white/70 text-[10px] font-semibold tracking-widest uppercase mb-1">
                  {category.count} {t("categoryShowcase.stylesSuffix")}
                </p>
                <p className="text-white text-base font-bold">
                  {t(`categoryShowcase.categories.${category.key}`)}
                </p>
              </div>
            </Link>
          ))}
